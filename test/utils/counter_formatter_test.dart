...
void main() {
  ...
  group('formatCount', () {
    test('binary renders R/G/B as three 8-bit lines', () {
      expect(formatCount(0x0000EE, .binary), '00000000\n00000000\n11101110');
    });
    test('octal is plain', () {
      expect(formatCount(8, .octal), '10');
    });
    test('decimal is ungrouped without a formatter', () {
      expect(formatCount(1234567, .decimal), '1234567');
    });
    test('decimal uses the provided grouping formatter', () {
      expect(
        formatCount(1234567, .decimal, formatDecimal: (c) => '1,234,567'),
        '1,234,567',
      );
    });
    test('hexadecimal is six uppercase digits without a hash', () {
      expect(formatCount(0x0000EE, .hexadecimal), '0000EE');
    });
  });
}
